...
// * * * * * * * * * * * * * * * * Private members  * * * * * * * * * * * * *//
Foam::scalarField 
Foam::implicitContactFvPatchVectorField::boundaryStiffness() const
{
...
Foam::implicitContactFvPatchVectorField::gapWidth() const
{
    const regionCoupledOFFBEATFvPatch& patch
        = refCast<const regionCoupledOFFBEATFvPatch>(regionCoupledPatch_);
...
void Foam::implicitContactFvPatchVectorField::correctWeights() 
{
    ...
    scalarField newSpringModulus = min(0.5*(deltaCoeffsP + deltaCoeffsN)*
        max(-gapWidth_, SMALL), 1.0)*contactSpringModulus_;

    scalarField newSpringModulus_t = neg(gapWidth_)*min(
        mag(slip_)/(0.5*(sqrt(magSfP) + sqrt(magSfN))), 1.0)*frictionSpringModulus_;

    // Spring resistance
    penaltyFact_ = neg(gapWidth_)*(
        relax_*newSpringModulus + (1 - relax_)*penaltyFact_);
    scalarField R_S = glued_ ?  magSfP/GREAT : magSfP/max(penaltyFact_, SMALL);

    // Spring resistance for tangent component
    penaltyFact_t_ = neg(gapWidth_)*(
        relax_*newSpringModulus_t + (1 - relax_)*penaltyFact_t_);
    scalarField R_S_t = glued_ ?  magSfP/GREAT : magSfP/max(penaltyFact_t_, SMALL);

    // Dashpot resistance
    scalarField R_D = magSfP/dashpotModulus_;
    ...
    vectorField contactGradExp_n(deltaCoeffsP*(1 - wC_)*(
            (Qp*R_P + Qn*R_N + pressure_*(R_P*nP + R_N*nN)) & (nP*nP)));
    vectorField contactGradExp_t(deltaCoeffsP*(1 - wC_t_)*(
            (Qp*R_P + Qn*R_N + pressure_*(R_P*nP + R_N*nN)) & (I - nP*nP)));
...
:
    ...
    frictionCoeff_(1),
    contactSpringModulus_(3.5e3),
    frictionSpringModulus_(3.5e3),
...
implicitContactFvPatchVectorField
(
    const implicitContactFvPatchVectorField& rcpvf,
    const fvPatch& p,
    const DimensionedField<vector, volMesh>& iF,
...
:
    ...
    contactSpringModulus_(rcpvf.contactSpringModulus_),
    frictionSpringModulus_(rcpvf.frictionSpringModulus_),
    dashpotModulus_(rcpvf.dashpotModulus_),
...
:
    ...
    contactSpringModulus_(readScalar(dict.lookup("contactSpringModulus"))),
    frictionSpringModulus_(readScalar(dict.lookup("frictionSpringModulus"))),
    dashpotModulus_(readScalar(dict.lookup("dashpotModulus"))),
    currentTime_(db().time().value()),
...
{    
    if (!isA<regionCoupledBaseOFFBEAT>(this->patch().patch()))
    {
        FatalErrorIn
        (
            "Foam::implicitContactFvPatchVectorField::implicitContactFvPatchVectorField"
            "("
            "    const fvPatch& p,"
            "    const DimensionedField<vector, volMesh>& iF,"
            "    const dictionary& dict"
...
void Foam::implicitContactFvPatchVectorField::autoMap
(
...
void Foam::implicitContactFvPatchVectorField::rmap
(
...
)
{
    ...
    const implicitContactFvPatchVectorField& dmptf =
        refCast<const implicitContactFvPatchVectorField>(ptf);
...
Foam::tmp<Foam::vectorField> Foam::implicitContactFvPatchVectorField::
snGrad() const
{
    const fvPatch& patch = this->patch();
...
void Foam::implicitContactFvPatchVectorField::updateCoeffs()
{
    if (updated())
    {
        return;
    ...
    const implicitContactFvPatchVectorField& nbr = this->neighbour();
        
    scalarField gapWidthPrev = patch.lookupPatchField<volScalarField, scalar>("gapWidth"); 
    ...
    if (!regionCoupledPatch_.owner())
    {
        ...
        gapWidth_ = gapWidth()*relax_ + gapWidthPrev*(1 - relax_);    
...
void Foam::implicitContactFvPatchVectorField::evaluate
(
...
Foam::implicitContactFvPatchVectorField::valueInternalCoeffs
(
...
Foam::tmp<Foam::vectorField>
Foam::implicitContactFvPatchVectorField::valueBoundaryCoeffs
(
    const tmp<scalarField>& w
) const
{
...
Foam::tmp<Foam::vectorField>
Foam::implicitContactFvPatchVectorField::gradientInternalCoeffs
(
    const scalarField&
) const
{
...
Foam::tmp<Foam::vectorField>
Foam::implicitContactFvPatchVectorField::gradientInternalCoeffs() const
{ 
...
Foam::tmp<Foam::vectorField>
Foam::implicitContactFvPatchVectorField::gradientBoundaryCoeffs
(
...
Foam::tmp<Foam::vectorField>
Foam::implicitContactFvPatchVectorField::gradientBoundaryCoeffs() const
{  
    return this->patch().deltaCoeffs()*(
...
#ifdef OPENFOAMFOUNDATION
void Foam::implicitContactFvPatchVectorField::updateInterfaceMatrix
(
    Field<scalar>& result,
...
void Foam::implicitContactFvPatchVectorField::manipulateMatrix(fvMatrix<vector>& matrix)
{
    const labelUList& faceCells = regionCoupledPatch_.faceCells();
...
Foam::tmp<Foam::Field<Foam::vector>>
Foam::implicitContactFvPatchVectorField::snGradTransformDiag() const
{
...
Foam::tmp<Foam::Field<Foam::vector>>
Foam::implicitContactFvPatchVectorField::tGradTransformDiag() const
{
...
void Foam::implicitContactFvPatchVectorField::write(Ostream& os) const
{
    ...
    writeEntry(os, "contactSpringModulus", contactSpringModulus_);
    writeEntry(os, "frictionSpringModulus", frictionSpringModulus_);
    writeEntry(os, "dashpotModulus", dashpotModulus_);
    ...
    os.writeEntry("contactSpringModulus", contactSpringModulus_);
    os.writeEntry("frictionSpringModulus", frictionSpringModulus_);
    os.writeEntry("dashpotModulus", dashpotModulus_);
