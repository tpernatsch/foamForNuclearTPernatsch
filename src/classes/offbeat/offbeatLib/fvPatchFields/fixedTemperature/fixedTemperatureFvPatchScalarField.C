    ...
    kappaName_(dict.lookupOrDefault<word>("kappa", "k"))
{
    if(valueRequired)
    {
        scalarField valueT = scalarField("value", dict, p.size());
        ...
        // equal to outerOxideTemperature
        if(dict.found("outerOxideTemperature"))
        {
            T_ = scalarField("outerOxideTemperature", dict, p.size());
        }
