...
void Foam::largeStrainUpdLag::correct()
{
    ...
    do
    {
        ...
        //     {
        //         DDEqn.diag() += (coefft*rDeltaT2)*mesh_.V()*mat_.rho().primitiveField();
        
        //         DDEqn.source() += rDeltaT2*mesh_.V()*mat_.rho().primitiveField()*
        //         (
        //             coefft0*DispPrev
        ...
        mechanicsSubSolver::correctedSigmaExp(
            (relJ_*(relFinv_ & sigma_))(), twoMuLambda, gradDD_);
        sigma_ = symm( ((sigmaExp_ + twoMuLambda*gradDD_) & relF_)/relJ_) ;
