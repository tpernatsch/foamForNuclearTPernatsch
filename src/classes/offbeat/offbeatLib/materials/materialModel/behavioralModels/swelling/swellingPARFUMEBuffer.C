...
#include "swellingPARFUMEBuffer.H"
...
namespace Foam
{
    defineTypeNameAndDebug(swellingPARFUMEBuffer, 0);
    addToRunTimeSelectionTable(swellingModel, swellingPARFUMEBuffer, dictionary);
}
...
Foam::swellingPARFUMEBuffer::swellingPARFUMEBuffer
(
    const fvMesh& mesh,
    ...
    perturb(1.0)
{
    ...
    Method_ = interpolateTableBase::interpolationMethodNames_["linear"];
    obMethod_ex_ = interpolateTableBase::outofBoundsMehtodNames_["extrapolation"];
    obMethod_fix_ = interpolateTableBase::outofBoundsMehtodNames_["fixed"];

    //- Read the temperature value
    tempValues_ = scalarField(dic.lookup("tempValus"));
    ...
    densityValue_ = scalarField(dic.lookup("densityValue"));
...
void Foam::swellingPARFUMEBuffer::correct
(
    const scalarField& T,
...
)
{
    ...
    if(fastFluence_ == nullptr or rho_==nullptr)
    {
    ...
    forAll(addr, addrI)
    {
        ...
        //- At density 1.96 g/cm3
        const scalar epsilon_ref = a1_iso+2.0*a2_iso*phi+3.0*a3_iso*pow(phi,2)+4.0*a4_iso*pow(phi,3);


        //- Isotropic scaling factor μ.
        scalar miu = table_epsilon_iso_ref(rho) / table_epsilon_iso_ref(1.96);

        scalar nominalValue(epsilon_ref*miu*flux*mesh_.time().deltaT().value());
        //- Here the strain is in unit of %
        swellingI[cellI] = swellingIOld[cellI] + (nominalValue/1e2*perturb)*I;
    }
