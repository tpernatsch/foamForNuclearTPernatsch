...
:
    mesh_(mesh),
    mat_(materials),
    heatSourceOptDict_(heatSourceOptDict),
    Q_
    (
        IOobject
        (
...
{
    // Initialize type for heatSource class
    word type;

    dictionary heatSourceOptDict
    (
...
void Foam::heatSource::correct()
{
    currentTime_ = mesh_.time().value();
}


Foam::scalar Foam::heatSource::nextDeltaT() 
{    
    ...
    if (deltaT > deltaTtoMarker)
    {
        ...
        Info<< "Maximum deltaT re-calculated by heatSource model: " 
            << mesh_.time().timeToUserTime(deltaT)
...
Foam::scalar Foam::heatSource::nextDeltaTAbsoluteChange(scalar deltaT) const
{
    scalar currentQ(this->averagePowerDensity());
    scalar predictedNewQ(this->predictNextPowerDensity());
