...
Foam::scalar Foam::GPLS::sigmaEq_func
(
...
Foam::symmTensor Foam::GPLS::cylSig
(
...
// viscoplastic strain rate function of the GPLS
Foam::scalar Foam::GPLS::G
(
...
)
{
   ...
   const scalar term1 = 1 + deltaT * one_m * deltaT * pDot / L(T, phi, p + theta*dEpsilonPEq);

   const scalar Lp = p + 1e-4;
...
*/
Foam::symmTensor Foam::GPLS::normal_tensor
(
    const scalar& T,
    const scalar& phi,
    const scalar& sigmaEq,
...
:
    ...
    tracePlasticN_// trace(n) should be egal to 0; validate the double dot product implementation
    (
        IOobject
        (
            "tr(plasticN)",
        ...
        mesh,
        dimensionedScalar("tr(plasticN)", dimless, 0)
    )
{  
    ...
    if(adjustTime)
    {
        maxAverageEpsilonPEq_ = controlDict.lookupOrDefault<scalar>(
            "maxAverageEpsilonPEq", GREAT);
        maxMaximumEpsilonPEq_ = controlDict.lookupOrDefault<scalar>(
            "maxMaximumEpsilonPEq", GREAT);
    }
...
)
{
    ...
    forAll(addr, addrI)
    {
        ...
        /// GPLS PARAMETERS. For tracking and validation
        ...
        sigmaEqH_[cellI] = sigmaEq_func(sigmaEff, Tm, phiValue_);
        ...
        if(sigmaEqH_[cellI] > 1.){
            ...
            tracePlasticN_[cellI] = tr(plasticNI[cellI]);
            ...
            //DEpsilonPEqI[cellI] = DEpsilonPEqOldTimeI[cellI];
 ...
 //               Info << "error -->" << error << endl;
        ...
        forAll(c, faceI)
        {
            ...
            // forAll(patchIDs, registeredID)
            {
                if (patchID > -1 and epsilonEl.boundaryField()[patchID].size())
                {
                    ...
                    scalar& sigmaEqHP = sigmaEqH_.boundaryFieldRef()[patchID][faceID];
                    sigmaEqHP = sigmaEq_func(sigmaEffP, TPm, phiValue_);
                    ...
                    if(sigmaEqHP > 1.){
                        ...
                        plasticNP[faceID] += normal_tensor(TPm, phiValue_, sigmaEqHP, sigmaEffP);

                        // Initialize Newton-Raphson
                        DEpsilonPEq_n = DEpsilonPEqP[faceID];
                        do{
                            //F
                            scalar F_n = F(sigmaEqHP, TPm, phiValue_, zetaValue_, 
                            epsilonPEqOldTimeP[faceID], DEpsilonPEq_n, deltaT, theta_);

                            //dF
                            scalar dF_n = dF(sigmaEqHP, TPm, phiValue_, 
                    ...
                    LP[faceID] = L(TPm, phiValue_, epsilonPEqOldTimeP[faceID]+DEpsilonPEq_n);
    ...
    Info << "EXITING INTEGRATOR" << endl;
    Info << "                     " << endl;
}


void Foam::GPLS::correctEpsilonEl
(
    volSymmTensorField& epsilonEl,
    const labelList& addr
)
...
void Foam::GPLS::updateTotalFields
(
    const labelList& addr
)
{
...
void Foam::GPLS::correctAdditionalStrain
(
...
Foam::scalar Foam::GPLS::nextDeltaT
(
    const labelList& addr
)
{
