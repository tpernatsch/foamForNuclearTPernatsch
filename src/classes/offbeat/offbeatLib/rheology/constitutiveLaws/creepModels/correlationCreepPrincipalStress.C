...
#include "correlationCreepPrincipalStress.H"
...
namespace Foam
{
    defineTypeNameAndDebug(correlationCreepPrincipalStress, 0);
...
Foam::correlationCreepPrincipalStress::correlationCreepPrincipalStress
(
    const fvMesh& mesh,
...
:
    ...
    A_(lawDict.lookup("coefficientList"))
{}
...
void Foam::correlationCreepPrincipalStress::correctCreep
(
...
)
{
    ...
    if(mesh_.time().timeIndex() > 0)
    {
        ...
        forAll(addr, addrI)
        {
            ...
            const scalar K = A_[0] + A_[1]*T + A_[2]*T*T;
            const scalar creepRate = K*sigmaEff*1e-6*(phi[cellI]*1e4/1e25*fcf_);
            ...
            forAll(c, faceI)
            {
...

                {
                    if (patchID > -1 and epsilonEl.boundaryField()[patchID].size())
                    {
                        ...
                        const scalar K = A_[0] + A_[1]*T + A_[2]*T*T;
                        const scalar creepRate = K*sigmaEff*1e-6*(phiP[faceID]*1e4/1e25*fcf_);
...
void Foam::correlationCreepPrincipalStress::computePrincipalStress
(
