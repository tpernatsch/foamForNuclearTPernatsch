...
namespace Foam
{
    defineTypeNameAndDebug(TobbeCreepModel, 0);
...
Foam::TobbeCreepModel::TobbeCreepModel
(
    const fvMesh& mesh,
...
void Foam::TobbeCreepModel::correctCreep
(
    ...
    if(mesh_.time().timeIndex() > 0)
    {
        ...
        do
        {
            ...
            forAll(addr, addrI)
            {
                ...
                //- TODO: (E.B.) implement the lookup to the clad type to choose
                //-              the appropriate correlation
                //- Tobbe correlation for 15-15 Ti austenitic - AIM1
                //scalar creepThRate = 
                //2.30e14*exp(-84600/(1.986*T))*Foam::sinh(39.72*sigmaEff/1e6/(1.986*T));
