...
//addTomesh_.time()SelectionTable(fluidModel, pimpleFluid, dictionary);
...
:
    fluidModel(typeName,const_cast<Foam::Time&>(mesh.time()),mesh, mesh.name()),
    Uf_(),
    stressTensor_
    (
        IOobject
    ...
    lambdaEffPtr_()
{
    ...
    if (mesh_.dynamic())
    {
        ...
        if
        (
            word(mesh_.ddtScheme("ddt(" + U().name() +')'))
...
//     tmp<vectorField> tvF
//     (
//         new vectorField(mesh_.boundary()[patchID].size(), vector::zero)
//     );
...
//     tmp<scalarField> tpF
//     (
//         new scalarField(mesh_.boundary()[patchID].size(), 0)
//     );
...
bool pimpleFluid::evolve()
{
    ...
    stressTensor_ = rho_*(-turbulence_->devReff()()-p*symmTensor(1,0,0,1,0,1));
...
void pimpleFluid::correctPhysics()
{
    evolve();
}

void pimpleFluid::correctTightlyCoupledPhysics()
{
    evolve();
}

scalar pimpleFluid::maxDeltaT()
{

    scalar maxDeltaT = mesh_.time().controlDict().lookupOrDefault<scalar>("maxDeltaT", GREAT);
    ...
    (
        min
        (
            deltaTFact*mesh_.time().deltaT().value(),
            maxDeltaT
        )
    );
...
) const
{
    ...
    tHTC() =
#endif
        (1.0/mesh_.deltaCoeffs().boundaryField()[patchID])/
        lambdaEffPtr_().boundaryField()[patchID];
