...
:
    constantFastFlux(mesh, materials, fastFluxDict),
    fluxData_(fastFluxDict.lookup("fastFlux")),
    timeData_(fastFluxDict.lookup("timePoints")),
    method_
    (
        interpolateTableBase::interpolationMethodNames_
        [
            fastFluxDict.lookupOrDefault<word>
    ...
    addr_()
{
    ...
    calcAddressing(fastFluxDict);
    calcMinMaxZ(fastFluxDict);

    axialModel_ = axialProfile::New
    (
        mesh, 
        fastFluxDict.subDict("axialProfile"), 
        addr_,
        pinDirection_,
        zMin_,
        zMax_
...
void Foam::timeDependentAxialProfile::correct()
{
    ...
    {
        calcAddressing(fastFluxDict_);
        calcMinMaxZ(fastFluxDict_);
    }
