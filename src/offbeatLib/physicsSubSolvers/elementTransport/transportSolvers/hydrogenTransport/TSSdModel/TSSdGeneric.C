...
namespace Foam
{
    defineTypeNameAndDebug(TSSdGeneric, 0);
...
:
    ...
    TSSd0_(readScalar(lawDict.lookup("TSSd0"))),
    Qd_(readScalar(lawDict.lookup("Qd")))
{
}
...
void Foam::TSSdGeneric::correctTSSd
(
...
)
{
    ...
    forAll(addr, addrI)
    {
        ...
        TSSd_[cellI] = TSSd0_ * exp(-Qd_ / (8.31446 * Ti[cellI]));
        ...
        forAll(c, faceI)
        {
            ...
            if (patchID > -1 and TSSd_.boundaryField()[patchID].size())
            {
                ...
                TSSdP[faceID] = TSSd0_ * exp(-Qd_ / (8.31446 * Tp[faceID]));
            }
        }
    }
