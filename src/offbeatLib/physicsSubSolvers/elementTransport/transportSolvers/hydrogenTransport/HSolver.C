...
namespace Foam
{
    defineTypeNameAndDebug(HSolver, 0);
...
void Foam::HSolver::updateCoefficients()
{
        ...
        if(mat_.materialsList()[i].name()=="Zircaloy")
        {
            ...
            forAll(addr, j)
            {
                ...
                if(kineticParameter_ == 0)
                {
                    ...
                    alpha_[cellI] = 3881  * exp(-82400 / 8.31446 * T_[cellI]);
                }
                else
                {
                ...
                forAll(c, faceI)
                {
                    ...
                    if (patchID > -1 and hydrideVolFrac_.boundaryField()[patchID].size())
                    {
                        ...
                        if(kineticParameter_ == 0)
                        {
                            ...
                            alphaP[faceID] =  3881 * exp(-82400 / 8.31446 * Tp[faceID]);
                        }
                        else
                        {
...
void Foam::HSolver::updateRateTerms()
{
    const labelListList& matAddrList(mat_.matAddrList());
    forAll(mat_.materialsList(), i)
    {
         if(mat_.materialsList()[i].name()=="Zircaloy")
        {
            ...
            forAll(addr, j)
            {
                ...
                {                   
                    CppEqnAdd_[cellI] = alpha_[cellI] * (Css_[cellI] - TSSp_[cellI]);
                ...
                forAll(c, faceI)
                {
                    ...
                    if (patchID > -1 and Css_.boundaryField()[patchID].size())
                    {
                        ...
                        const scalarField& TSSpP(TSSp_.boundaryFieldRef()[patchID]);
                        ...
                        if(CssP[faceID] > TSSpP[faceID] && hydrideVolFracP[faceID] <= 0.99)
                        {   
                            CppEqnAddP[faceID] = alphaP[faceID] * (CssP[faceID] - TSSpP[faceID]);
                            CssEqnAddP[faceID] = -CppEqnAddP[faceID];
                        }
                        else if(TSSpP[faceID] >= CssP[faceID] && CssP[faceID] > TSSdP[faceID])
                        {
                            CssEqnAddP[faceID] = 0;
                            CppEqnAddP[faceID] = 0;
                        }
