...
namespace Foam
{
    defineTypeNameAndDebug(TSSpMCMINN, 0);
...
void Foam::TSSpMCMINN::correctTSSp
(
...
)
{
    ...
    const scalar TSSp0 = 1.39e5;
    const scalar Qd = 34470;

    forAll(addr, addrI)
    {
        const label cellI = addr[addrI];

        TSSp_[cellI] = TSSp0 * exp(-Qd / (8.31446 * Ti[cellI]));
        ...
        forAll(c, faceI)
        {
            ...
            if (patchID > -1 and TSSp_.boundaryField()[patchID].size())
            {
                ...
                TSSpP[faceID] = TSSp0 * exp(-Qd / (8.31446 * Tp[faceID]));
            }
        }
    }
