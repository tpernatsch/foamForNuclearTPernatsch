...
Foam::label Foam::AMIInterpolationOFFBEAT::calcOverlappingProcs
(
...
void Foam::AMIInterpolationOFFBEAT::distributePatches
(
...
void Foam::AMIInterpolationOFFBEAT::distributeAndMergePatches
(
    const mapDistribute& map,
    const primitivePatch& tgtPatch,
    const globalIndex& gi,
...
Foam::AMIInterpolationOFFBEAT::calcProcMap
(
    const primitivePatch& srcPatch,
    const primitivePatch& tgtPatch
) const
{
    ...
    forAll(procBb[Pstream::myProcNo()], bbI)
    {
        ...
        //- NOTE: Modified with respect to normal AMI
        //- 0.01 --> 0.1
        bb.inflate(0.1);
    }
