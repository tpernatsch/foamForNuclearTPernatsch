...
namespace Foam
{
    defineTypeNameAndDebug(AMIMethodOFFBEAT, 0);
    defineRunTimeSelectionTable(AMIMethodOFFBEAT, components);
}


// * * * * * * * * * * * * Protected Member Functions  * * * * * * * * * * * //

void Foam::AMIMethodOFFBEAT::checkPatches() const
{
...
bool Foam::AMIMethodOFFBEAT::initialise
(
...
void Foam::AMIMethodOFFBEAT::writeIntersectionOBJ
(
    const scalar area,
    const face& f1,
    const face& f2,
...
void Foam::AMIMethodOFFBEAT::resetTree()
{
    // Clear the old octree
    treePtr_.clear();

    treeBoundBox bb(tgtPatch_.points(), tgtPatch_.meshPoints());
    // NOTE: different than normal AMI method
    // necessary for large gaps
    bb.inflate(0.1);
...
Foam::label Foam::AMIMethodOFFBEAT::findTargetFace
(
...
void Foam::AMIMethodOFFBEAT::appendNbrFaces
(
    const label facei,
...
Foam::scalar Foam::AMIMethodOFFBEAT::maxWalkAngle() const
{
    return degToRad(89);
}
...
Foam::AMIMethodOFFBEAT::AMIMethodOFFBEAT
(
...
bool Foam::AMIMethodOFFBEAT::conformal() const
{
    return true;
}
