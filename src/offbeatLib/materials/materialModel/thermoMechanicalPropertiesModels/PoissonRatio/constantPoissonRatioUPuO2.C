...
#include "constantPoissonRatioUPuO2.H"
...
namespace Foam
{
    defineTypeNameAndDebug(constantPoissonRatioUPuO2, 0);
...
Foam::constantPoissonRatioUPuO2::constantPoissonRatioUPuO2
(
    const fvMesh& mesh,
    ...
    mapper_(nullptr)
{
    ...
    if((dict.found("PoissonRatio")) || (dict.dictName() == "PoissonRatio"))
    {
        ...
        PoissonRatioValue_ = 
        PoissonRatioDict.lookupOrDefault<scalar>("PoissonRatioValue", 0.276);
    }

    if(isotropicCracking_)
    {        
        WarningIn("constantPissonRatioUPuO2(const fvMesh&, const dictionary&, const word)")
...
// * * * * * * * * * * * * * * Member Functions  * * * * * * * * * * * * * * //
void Foam::constantPoissonRatioUPuO2::correct
(
    scalarField& sf, 
    const scalarField& T, 
...
)
{
    ...
    {   
        ...
        if(isotropicCracking_)
        {
            ...
            {
                sf[cellI] =  pow(0.5, nCracks_.internalField()[cellI])*PoissonRatioValue_;                
            }
        }
        else
        {
