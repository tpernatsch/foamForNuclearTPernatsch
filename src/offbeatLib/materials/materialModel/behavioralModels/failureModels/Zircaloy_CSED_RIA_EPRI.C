...
#include "Zircaloy_CSED_RIA_EPRI.H"
...
namespace Foam
{
    defineTypeNameAndDebug(Zircaloy_CSED_RIA_EPRI, 0);
    addToRunTimeSelectionTable(failureModel, Zircaloy_CSED_RIA_EPRI, dictionary);
}
...
Foam::Zircaloy_CSED_RIA_EPRI::Zircaloy_CSED_RIA_EPRI
(
    const fvMesh& mesh,
...
:
    ...
    CH_Value_(readScalar(failureModelDict_.lookup("CH"))),
    crackLengthValue_(readScalar(failureModelDict_.lookup("crackLength")))
{    
}
...
bool Foam::Zircaloy_CSED_RIA_EPRI::isFailed
(
