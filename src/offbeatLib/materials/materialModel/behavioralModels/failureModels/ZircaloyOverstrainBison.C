...
#include "ZircaloyOverstrainBison.H"
...
namespace Foam
{
    defineTypeNameAndDebug(ZircaloyOverstrainBison, 0);
    addToRunTimeSelectionTable(failureModel, ZircaloyOverstrainBison, dictionary);
}
...
Foam::ZircaloyOverstrainBison::ZircaloyOverstrainBison
(
    const fvMesh& mesh,
...
bool Foam::ZircaloyOverstrainBison::isFailed
(
...
)
{
    ...
    forAll( patchNames_, i )
    {
        ...
        //- Collect failure from all MPI processes
        reduce(patchFailed, orOp<bool>());
        ...
        if (patchFailed)
        {
            ...
            if (faceIndex >= 0)
            {
                failedMaterial_().boundaryFieldRef()[patchID][faceIndex] = 1;
            }
        }
    }
