...
{
    // Default pick type
    word phaseTransitionModelName = defaultModel;

    // Prepare model dict
    dictionary modelDict
    (
        dict.subOrEmptyDict("phaseTransition")
    );

    if(dict.found("phaseTransitionModel"))
    {
        dict.lookup("phaseTransitionModel") >> phaseTransitionModelName;

        // Revert modelDict to materialDict
        modelDict = dict;        
            
        WarningIn("phaseTransitionModel::New(const fvMesh&, const dictionary&, const word)")
            << "Keyword 'phaseTransitionModel' is deprecated. " 
            << "Please create a 'phaseTransition' dictionary instead, where the typeName of the model is expressed via a keyword 'type'." << nl << endl;
    }
    else
    {
        if(dict.found("phaseTransition"))
        {
            modelDict.lookup("type") >> phaseTransitionModelName;
        }
        else
        {
    ...
    if (debug)
    {
        Info<< "Selecting phaseTransition type "
            << phaseTransitionModelName << endl;
    }

    return autoPtr<phaseTransitionModel>(cstrIter()(mesh, modelDict, dict));
}
