...
namespace Foam
{
    ...
    const char* swellingCorrelationPyC::group_ = 
        ("behavioralModels::swelling::PyCCorrelation");

    defineParameter(swellingCorrelationPyC, F_epsilonSwelling_, 
        "F_epsilonSwelling", (dimless), 1.0);
        
    defineParameter(swellingCorrelationPyC, delta_epsilonSwelling_, 
...
)
{
    ...
    forAll(addr, addrI)
    {
        ...
        if (sp_ == true)
        {
          ...
          swellingI[cellI] = nominalValue_sphere;
        }
        else
        {
