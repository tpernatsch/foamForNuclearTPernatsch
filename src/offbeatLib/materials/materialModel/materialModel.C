...
:
    ...
    PoissonRatio_(),
    thermalExpansion_(),
    damageModel_(damageModel::New(mesh, materialModelDict))
{}
...
)
{
    ...
    if(materialModelDict.found("material"))
    {
        materialModelDict.lookup("material") >> materialName;
            
        WarningIn("materialModel::New(const fvMesh&, const dictionary&, const labelList&)")
            << "Material keyword 'material' is deprecated. " 
            << "Please use 'type' instead." << nl << endl;
    }
    else
    {
        materialModelDict.lookup("type") >> materialName;
    }
...
void Foam::materialModel::soften
(
    scalarField& E, 
    scalarField& nu
) 
{
    damageModel_->soften(E, nu, addr_);
}


void Foam::materialModel::correctDamage
(
...
{
    damageModel_->correct(sigmaHyd, sigmaDev, epsilonEl, addr_);
}
