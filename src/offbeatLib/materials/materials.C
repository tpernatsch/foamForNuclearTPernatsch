...
)
{
    ...
    word type = solverDict.lookupOrDefault<word>("materialProperties", "byZone");
...
Foam::scalar Foam::materials::nextDeltaT()
{
    ...
    const scalar dRhoPct = gMax((rhoi - rhoOld) / rhoOld * 100); 
    const scalar dCpPct = gMax((Cpi - CpOld) / rhoOld * 100);
    const scalar dKPct = gMax((ki - k_Old) / k_Old * 100);
