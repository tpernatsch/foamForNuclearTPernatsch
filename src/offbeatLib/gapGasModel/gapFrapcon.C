...
void Foam::gapFrapcon::correctScalingFactors()
{
    ...
    pointVectorField pointsDisplacement = meshPointInterpolation.interpolate(DorDD_);
    ...
    for(const label& patchID : gapPatchIDs) 
    {
        ...
        const regionCoupledOFFBEATFvPatch& patch(refCast<const regionCoupledOFFBEATFvPatch>(p));
        
        if(patch.owner()) {
            // NOTE: fuel is supposed to be the owner
            // so, if fuel => skip this patch
            continue;
        }
        ...
        forAll(patch, faceID)
        {
            ...
            scalar maxAxialLocation = max(p.patch()[faceID].points(pointsDisplaced) & pinDirection_);
            scalar minAxialLocation = min(p.patch()[faceID].points(pointsDisplaced) & pinDirection_);
            ...
            if(cutByTopPlane || cutByBottomPlane)
            {
                ...
                forAll(edges, edgeID)
                {
                    ...
                    totalFaceLabels.append(totalFaceLabels.size());
                    ...
                    (
                        ...
                        (plBottom.sideOfPlane(base) != Foam::plane::side::FLIP)
                    )
                    {
                        // Append to bottom list
                        bottomPlenumFacePts.append(base);
                        bottomPlenumFaceLabels.append(bottomPlenumFaceLabels.size());
                        ...
                        if (distToTop <= 1.0 & distToTop >= 0)
                        {
                            ...
                            gapFaceLabels.append(gapFaceLabels.size());

                            // Append to top list 
                            topPlenumFacePts.append(base + distToTop*dir);
                            topPlenumFaceLabels.append(topPlenumFaceLabels.size());
                        }
                    }
                    else if
                    (
                        // If point is above top plane
                        (plTop.sideOfPlane(base) != Foam::plane::side::FLIP)
                    )
                    {
                ...
                scalar overlapArea = calcPtsFieldArea(gapFacePts, gapFaceLabels);
                scalar totalArea   = calcPtsFieldArea(totalFacePts, totalFaceLabels);
                scalar topPlenumArea = calcPtsFieldArea(topPlenumFacePts, topPlenumFaceLabels);
                scalar bottomPlenumArea = calcPtsFieldArea(bottomPlenumFacePts, bottomPlenumFaceLabels);
...
Foam::scalar Foam::gapFrapcon::calcPtsFieldArea(const List<point>& facePts, const labelList& faceLabels) {
    pointField ptsField(facePts);
    scalar area;
    if ( facePts.size() == 0 ) {
        area = 0.0;
    } else {
        face the_face(faceLabels);
#ifdef OPENFOAMFOUNDATION
        area = (mag(the_face.area(ptsField)));
#elif OPENFOAMESI
        area = (mag(the_face.areaNormal(ptsField)));
#endif
    }
    return area;
}
...
(const std::set<label>& patchIDs)
{
    ...
    for(const label& patchID : patchIDs)
    {
        ...
        const fvPatchVectorField& Dp(DorDD_.boundaryField()[patchID]);
        const vectorField Cf(patch.faceCentres());
...
:
    ...
    zMin_(-GREAT),  
    includeCentralHole_
    (gapGasDict.lookupOrDefault<bool>("includeCentralHole", true)),
    relocationName_
    (gapGasDict.lookupOrDefault<word>("relocationName", "epsilonRelocation"))
{
