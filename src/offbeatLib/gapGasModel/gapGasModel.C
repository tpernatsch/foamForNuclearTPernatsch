...
)
{
    ...
    dictionary gapGasDict
    (
        solverDict.subOrEmptyDict("gapGasOptions")
    );

    if (solverDict.found("gapGas"))
    {
        if (solverDict.isDict("gapGas"))
        {
            // Case 1: gapGas is a dictionary
            gapGasDict = solverDict.subDict("gapGas");
            gapGasDict.lookup("type") >> type;
        }
        else
        {
            // Case 2: gapGas is a word (type)
            solverDict.lookup("gapGas")  >> type;
            // gapGasDict stays as gapGasOptions
        }
    }
