...
:
    ...
    scalingFactorData_(dict.found("axialScalingFactors") ?
        dict.lookup("axialScalingFactors") :
        scalarField(zValues_.size(), 1.0)),
    ...
    scalingFactorTable_(zValues_, scalingFactorData_, zMethod_)
{
    ...
    forAll(addr_, i)
    {
        scalingFactorProfile_[i] = scalingFactorTable_(z[i]);
    }
}
