...
#include "regionCoupledTemperatureFvPatchScalarField.H"
#include "volFields.H"
...
:
    ...
    kappaName_(dict.lookupOrDefault<word>("kappa", "unknown")),
...
regionCoupledTemperatureFvPatchScalarField
(
    const regionCoupledTemperatureFvPatchScalarField& psf,
    const fvPatch& p,
    const DimensionedField<scalar, volMesh>& iF,
...
Foam::regionCoupledTemperatureFvPatchScalarField::kappa() const
{
    const fvMesh& mesh = patch().boundaryMesh().mesh();
    const label patchi = patch().index();

    const word& phase(this->internalField().group());
    ...
    else
    {
        ...
        return tmp<scalarField>(new scalarField(kappa));
    }
}

void Foam::regionCoupledTemperatureFvPatchScalarField::updateCoeffs()
{
    if (updated())
    {
        return;
    ...
    typedef regionCoupledTemperatureFvPatchScalarField thisType;
...
void Foam::regionCoupledTemperatureFvPatchScalarField::write
(
    Ostream& os
) const
{
    ...
    writeEntry(os, "enabled", enabled_);
}
