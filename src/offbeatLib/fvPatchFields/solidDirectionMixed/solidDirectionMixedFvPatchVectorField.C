...
void solidDirectionMixedFvPatchVectorField::evaluate(const Pstream::commsTypes)
{
    ...
    Field<vector> normalValue = transform(valueFraction(), refValue() + w_*this->patchInternalField());
...
solidDirectionMixedFvPatchVectorField::snGrad() const
{
    ...
    Field<vector> normalValue =
        transform(this->valueFraction(), this->refValue() + w_*this->patchInternalField());
...
tmp<vectorField>
solidDirectionMixedFvPatchVectorField::gradientInternalCoeffs() const
{
    return -(1 - w_)*this->patch().deltaCoeffs()*snGradTransformDiag();
}
