...
)
{
    ...
    if (solverDict.found("rheology"))
    {
        if (solverDict.isDict("rheology"))
        {
            // Case 1: rheology is a dictionary
            rheologyDict = solverDict.subDict("rheology");
            rheologyDict.lookup("type") >> type;
        }
        else
        {
            // Case 2: rheology is a word (type)
            solverDict.lookup("rheology") >> type;
            // rheologyDict stays as rheologyOptions
        }
    }
