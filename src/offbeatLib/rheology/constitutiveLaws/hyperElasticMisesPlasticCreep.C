...
)
{
    ...
    {
        const label cellI = addr[addrI];
        averageDepsilonPEq += (DepsilonCreepEqI[cellI])*V[cellI];
        volTot += V[cellI];

        maxDepsilonPEq = max(maxDepsilonPEq, (DepsilonCreepEqI[cellI]));
    }
    ...
    Info<< "Maximum deltaT calculated by hyperElasticMisesPlasticCreep model: " 
