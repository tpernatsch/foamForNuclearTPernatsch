...
#include "constantCreepPrincipalStress.H"
...
namespace Foam
{
    defineTypeNameAndDebug(constantCreepPrincipalStress, 0);
    addToRunTimeSelectionTable
    (
        creepModel,
        constantCreepPrincipalStress,
        dictionary
    );
}
...
Foam::constantCreepPrincipalStress::constantCreepPrincipalStress
(
    const fvMesh& mesh,
...
:
    ...
    K_(readScalar(lawDict.lookup("creepCoefficient")))
{}
...
void Foam::constantCreepPrincipalStress::correctCreep
(
...
)
{
    ...
    if(mesh_.time().timeIndex() > 0)
    {
        ...
        forAll(addr, addrI)
        {
            ...
            const scalar sigmaEff = max(min(p1-vc_*(p2+p3)-3*mu[cellI]*DepsilonCreepEq_.ref()[cellI], 3e8), 0.0);
            ...
            const scalar creepRate = K_*sigmaEff*1e-6*(phi[cellI]*1e4/1e25*fcf_);
            ...
            forAll(c, faceI)
            {
...

                {
                    if (patchID > -1 and epsilonEl.boundaryField()[patchID].size())
                    {
                        ...
                        const scalar sigmaEff = max(min(p1p-vc_*(p2p+p3p)-3*muP[faceID]*DepsilonCreepEqP[faceID], 3e8), 0.0);
                        const scalar DepsilonCreepEqPPrev = DepsilonCreepEqP[faceID];

                        // * # * # * # * # * # * # * # * # * # * # * # * # * # * # * # * #
                        //- Calculate creep strain rate with power law
                        const scalar creepRate = K_*sigmaEff*1e-6*(phiP[faceID]*1e4/1e25*fcf_);
...
void Foam::constantCreepPrincipalStress::computePrincipalStress
(
