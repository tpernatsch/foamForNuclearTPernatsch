...
namespace Foam
{
    defineTypeNameAndDebug(ZircaloyLimbackLoca, 0);
    ...
    const char* ZircaloyLimbackLoca::group_ = 
        ("constitutiveLaws::creep::ZircaloyLimbackLoca");
        
    defineParameter(ZircaloyLimbackLoca, F_DepsilonCreepIrrEq_, 
        "F_DepsilonCreepIrrEq", (dimless), 1.0);        
    defineParameter(ZircaloyLimbackLoca, delta_DepsilonCreepIrrEq_, 
        "delta_DepsilonCreepIrrEq", (dimless), 0.0);        
        
    defineParameter(ZircaloyLimbackLoca, F_DepsilonCreepThEq_, 
        "F_DepsilonCreepThEq", (dimless), 1.0);        
    defineParameter(ZircaloyLimbackLoca, delta_DepsilonCreepThEq_, 
        "delta_DepsilonCreepThEq", (dimless), 0.0);
                
    defineParameter(ZircaloyLimbackLoca, F_DepsilonCreepPrimEq_, 
        "F_DepsilonCreepPrimEq", (dimless), 1.0);        
    defineParameter(ZircaloyLimbackLoca, delta_DepsilonCreepPrimEq_, 
        "delta_DepsilonCreepPrimEq", (dimless), 0.0);
}
...
Foam::ZircaloyLimbackLoca::ZircaloyLimbackLoca
(
    const fvMesh& mesh,
...
:
    ...
    primaryCreepAbove700K_(false),
    LOCACreep_(lawDict.lookupOrDefault<bool>("LOCACreep", true)),
    ...
    C2(1.0)
{
    if(primaryCreep_)
    {
        primaryCreepAbove700K_ = lawDict.lookupOrDefault<Switch>(
            "primaryCreepAbove700K", true);
    }
...
void Foam::ZircaloyLimbackLoca::correctCreep
(
...
)
{
    ...
    const scalar& F_DepsilonCreepIrrEq = this->F_DepsilonCreepIrrEq().value();
    const scalar& F_DepsilonCreepThEq = this->F_DepsilonCreepThEq().value();
    const scalar& F_DepsilonCreepPrimEq = this->F_DepsilonCreepPrimEq().value();

    const scalar& delta_DepsilonCreepIrrEq = this->delta_DepsilonCreepIrrEq().value();
    const scalar& delta_DepsilonCreepThEq = this->delta_DepsilonCreepThEq().value();
    const scalar& delta_DepsilonCreepPrimEq = this->delta_DepsilonCreepPrimEq().value();
    ...
    {        
        ...
        else
        {
            FatalErrorInFunction
                ...
                << "ZircaloyLimbackLoca but not found in registry." << nl
                << abort(FatalError);
        }
    ...
    if(mesh_.time().timeIndex() > 0)
    {
        ...
        volScalarField creepThermalRateOld(DepsilonCreepThEq_.oldTime()/max(mesh_.time().deltaT0Value()/3600, SMALL));
        ...
        do
        {
            ...
            forAll(addr, addrI)
            {
                ...
                if (irradiationCreep_)
                {
                    ...
                    creepIrrRate = calcIrradiationCreepRate(phi, sigmaEff)*F_DepsilonCreepIrrEq
                        + delta_DepsilonCreepIrrEq;

                    if (NewtonRaphsonMethod_)
                    {
                        creepIrrRatePrime = calcIrradiationCreepRatePrime(phi, sigmaEff, 
                            creepIrrRate)*F_DepsilonCreepIrrEq;
                    }
                ...
                else if( (T > 700 and T < 900) and LOCACreep_ )
                {
                    // Compute thermal creep in normal operating conditions:
                    ...
                    scalar cTh900 = calcLOCAThermalCreepRate(
                        900, betaFrac, sigmaEff, creepThermalRateOld[cellI]);

                    // Interpolation:
                    creepThRate = exp(log(cTh700) + (
                        (T-700)/(900-700) * log(cTh900/cTh700)));

                    if (NewtonRaphsonMethod_)
                    {
                        // Compute derivative of thermal creep in normal operating conditions:
                        scalar cThPrime700 = calcNormalThermalCreepRatePrime(
                            fastFluenceI_[cellI], 700, sigmaEff, cTh700);

                        // Compute derivative of thermal creep in LOCA conditions (Erbacher):
                        scalar cThPrime900 = calcLOCAThermalCreepRatePrime(
                            900, betaFrac, sigmaEff, creepThermalRateOld[cellI]);

                        creepThRatePrime = exp(log(cThPrime700) + (
                            (T-700)/(900-700) * log(cThPrime900/cThPrime700)));
                    }
                }
                else if( T >= 900 and LOCACreep_)
                {
                    // Compute thermal creep in LOCA conditions (Erbacher):
                    creepThRate = calcLOCAThermalCreepRate(
                        T, betaFrac, sigmaEff, creepThermalRateOld[cellI]);
                ...
                creepThRate = creepThRate*F_DepsilonCreepThEq + delta_DepsilonCreepThEq;

                if (NewtonRaphsonMethod_)
                {
                    creepThRatePrime = creepThRatePrime*F_DepsilonCreepThEq;

                    // Update thermal creep equivalent
                    DepsilonCreepThEq_.ref()[cellI] = 
                    (1 - relax_)*DepsilonCreepThEq_.ref()[cellI] + 
                    relax_*(DepsilonCreepThEq_.ref()[cellI] - 
                        (DepsilonCreepThEq_.ref()[cellI] - creepThRate*deltaT)/(1 + 3*mu[cellI]/1e6*creepThRatePrime*deltaT));
                }
                else
                {
                    // Update thermal creep equivalent
                ...
                creepThRate = DepsilonCreepThEq_.ref()[cellI]/deltaT;
                ...
                if (primaryCreep_ & primaryCreepAbove700K_)
                {                    
                    creepPrimRate = calcPrimaryCreepRate(creepIrrRate, creepThRate, 
                        epsilonCreepPrimEqOld_[cellI])*F_DepsilonCreepPrimEq 
                        + delta_DepsilonCreepPrimEq;
                ...
                forAll(c, faceI)
                {
                    const label patchID = mesh_.boundaryMesh().whichPatch(c[faceI]);
                    {
                        if (patchID > -1 and epsilonEl.boundaryField()[patchID].size())
                        {
                            ...
                            scalarField& creepThermalRateOldP = 
                            creepThermalRateOld.boundaryFieldRef()[patchID];
                        ...
                        /****** Calculate secondary thermal creep rate [1/hr] ********/
                            ...
                            else if( (T > 700 and T < 900) and LOCACreep_)
                            {
                                ...
                                // Compute thermal creep in LOCA conditions (Erbacher):
                                scalar cTh900 = calcLOCAThermalCreepRate(
                                    900, betaFrac, sigmaEff, creepThermalRateOldP[faceID]);
                            ...
                            else if ( T >= 900 and LOCACreep_ )
                            {
                                // Compute thermal creep in LOCA conditions (Erbacher):
                                creepThRate = calcLOCAThermalCreepRate(
                                    T, betaFrac, sigmaEff, creepThermalRateOldP[faceID]);
                            ...
                            if (NewtonRaphsonMethod_)
                            {
                                ...
                                (1 - relax_)*DepsilonCreepThEqP[faceID] + relax_*(DepsilonCreepThEqP[faceID] - 
                                (DepsilonCreepThEqP[faceID] - creepThRate*deltaT)/(1 + 3*muP[faceID]/1e6*creepThRatePrime*deltaT));
                            }
                            else
                            {
                                // Update thermal creep equivalent
                            ...
                            creepThRate = DepsilonCreepThEqP[faceID]/deltaT;
                            ...
                            {                
                                creepPrimRate = calcPrimaryCreepRate(creepIrrRate, creepThRate, 
                                    epsilonCreepPrimEqOldP[faceID])*F_DepsilonCreepPrimEq
                                    + delta_DepsilonCreepPrimEq;
...
Foam::scalar Foam::ZircaloyLimbackLoca::calcIrradiationCreepRate
(
    const scalar& flux,
    const scalar& sigmaEff
)
...
Foam::scalar Foam::ZircaloyLimbackLoca::calcIrradiationCreepRatePrime
(
    const scalar& flux,
    const scalar& sigmaEff,
...
Foam::scalar Foam::ZircaloyLimbackLoca::calcNormalThermalCreepRate
(
...
Foam::scalar Foam::ZircaloyLimbackLoca::calcNormalThermalCreepRatePrime
(
    const scalar& fastFluence,
    const scalar& T,
    const scalar& sigmaEff,
...
Foam::scalar Foam::ZircaloyLimbackLoca::calcPrimaryCreepRate
(
    const scalar& creepIrrRate,
    const scalar& creepThRate,
...
Foam::scalar Foam::ZircaloyLimbackLoca::calcPrimaryCreepRatePrime
(
    const scalar& creepIrrRate,
    const scalar& creepThRate,
...
)
{
    ...
    if(creepSecRate > VSMALL)
    {
        ...
        if(epsilonCreepPrimEqOld < (epsilonPrim_sat-SMALL))
        {
            ...
            scalar arg = min(D*creepSecRate, 355.0);
...
Foam::scalar Foam::ZircaloyLimbackLoca::calcLOCAThermalCreepRate
(
    const scalar& T,
...
Foam::scalar Foam::ZircaloyLimbackLoca::calcLOCAThermalCreepRatePrime
(
    const scalar& T,
    const scalar& betaFrac,
    const scalar& sigmaEff,
...
)
{
    ...
    const scalar creepRate = calcLOCAThermalCreepRate(
        T, betaFrac, sigmaEff, creepRateOld);
    ...
    // Mixed phase, creep strain rate <= 3e-3 (s^-1)
    else if( creepRateOld/3600 <= 3e-3 )
    {
        // Useful quantities for the interpolation:
        const scalar nMix   = 2.33;
