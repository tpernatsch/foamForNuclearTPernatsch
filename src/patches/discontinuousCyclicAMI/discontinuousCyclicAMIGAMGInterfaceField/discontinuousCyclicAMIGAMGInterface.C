...
#include "discontinuousCyclicAMIGAMGInterface.H"
...
namespace Foam
{
    defineTypeNameAndDebug(discontinuousCyclicAMIGAMGInterface, 0);
    addToRunTimeSelectionTable
    (
        GAMGInterface,
        discontinuousCyclicAMIGAMGInterface,
        lduInterface
    );
    addToRunTimeSelectionTable
    (
...
Foam::discontinuousCyclicAMIGAMGInterface::discontinuousCyclicAMIGAMGInterface
(
    const label index,
...
:
    ...
    neighbPatchID_
    (
        refCast<const discontinuousCyclicAMILduInterface>(fineInterface).neighbPatchID()
    ),
    owner_
    (
        refCast<const discontinuousCyclicAMILduInterface>(fineInterface).owner()
    ),
    forwardT_
    (
        refCast<const discontinuousCyclicAMILduInterface>(fineInterface).forwardT()
    ),
    reverseT_
    (
        refCast<const discontinuousCyclicAMILduInterface>(fineInterface).reverseT()
    ),
    myProcNo_(-1)
{
    const auto& finediscontinuousCyclicAMIInterface =
        refCast<const discontinuousCyclicAMILduInterface>(fineInterface);
    ...
    if (finediscontinuousCyclicAMIInterface.owner())
    {
        ...
        (
            ...
            (
                finediscontinuousCyclicAMIInterface.AMI(),
...
:
    ...
    (
        ...
        (
            refCast
            <
                const discontinuousCyclicAMILduInterface
    ...
    myProcNo_(-1)
{
    ...
    forAll(allInterfaces, inti)
    {
        ...
        {
            const auto& intf =
                refCast<const discontinuousCyclicAMIGAMGInterface>(allInterfaces[inti]);
    ...
    if (singlePatchProc == -1)
    {
        ...
        labelListList newToOldRanks;
        {
            ...
            forAll(allInterfaces, inti)
            {
                if (allInterfaces.set(inti))
                {
                    const auto& intf = refCast<const discontinuousCyclicAMIGAMGInterface>
                    (
                        allInterfaces[inti]
                    );
...
Foam::tmp<Foam::labelField> Foam::discontinuousCyclicAMIGAMGInterface::internalFieldTransfer
(
    const Pstream::commsTypes commsType,
    const labelUList& iF
) const
{
    const discontinuousCyclicAMIGAMGInterface& nbr =
        dynamic_cast<const discontinuousCyclicAMIGAMGInterface&>(neighbPatch());
...
void Foam::discontinuousCyclicAMIGAMGInterface::write(Ostream& os) const
{
