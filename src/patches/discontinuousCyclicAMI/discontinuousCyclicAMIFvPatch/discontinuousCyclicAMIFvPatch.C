...
namespace Foam
{
    defineTypeNameAndDebug(discontinuousCyclicAMIFvPatch, 0);
    addToRunTimeSelectionTable(fvPatch, discontinuousCyclicAMIFvPatch, polyPatch);
...
// void Foam::discontinuousCyclicAMIFvPatch::newInternalProcFaces
// (
//     label& newFaces,
//     label& newProcFaces
// ) const
...
void Foam::discontinuousCyclicAMIFvPatch::makeWeights(scalarField& w) const
{
    if (coupled())
    {
        const discontinuousCyclicAMIFvPatch& nbrPatch = neighbFvPatch();
...
void Foam::discontinuousCyclicAMIFvPatch::makeDeltaCoeffs(scalarField& coeffs) const
{
    // Apply correction to default coeffs
}


void Foam::discontinuousCyclicAMIFvPatch::makeNonOrthoDeltaCoeffs(scalarField& coeffs) const
{
    // Apply correction to default coeffs
    //coeffs = Zero;
}
...
void Foam::discontinuousCyclicAMIFvPatch::makeNonOrthoCorrVectors(vectorField& vecs) const
{
...
Foam::tmp<Foam::vectorField> Foam::discontinuousCyclicAMIFvPatch::delta() const
{
    const discontinuousCyclicAMIFvPatch& nbrPatch = neighbFvPatch();
...
Foam::tmp<Foam::labelField> Foam::discontinuousCyclicAMIFvPatch::interfaceInternalField
(
...
Foam::tmp<Foam::labelField> Foam::discontinuousCyclicAMIFvPatch::internalFieldTransfer
(
...
void Foam::discontinuousCyclicAMIFvPatch::movePoints()
{
    if (!owner() || !discontinuousCyclicAMIPolyPatch_.createAMIFaces())
    {
    ...
    const_cast<vectorField&>(Sf()) = discontinuousCyclicAMIPolyPatch_.faceAreas();
    const_cast<vectorField&>(Cf()) = discontinuousCyclicAMIPolyPatch_.faceCentres();
    const_cast<scalarField&>(magSf()) = mag(Sf());

    const discontinuousCyclicAMIFvPatch& nbr = neighbPatch();
    ...
    if (discontinuousCyclicAMIPolyPatch_.owner())
    {
        scalarField& phip = meshPhiBf[patch().index()];
        forAll(phip, facei)
        {
            const face& f = discontinuousCyclicAMIPolyPatch_.localFaces()[facei];
            ...
            const scalar geomArea = f.mag(discontinuousCyclicAMIPolyPatch_.localPoints());
