...
template<template<class> class Field, class Type>
Foam::IOFieldField<Field,Type>::IOFieldField(const IOobject& io, const label size)
:
    regIOobject(io),
    timeIndex_(this->time().timeIndex()),
    field0Ptr_(NULL)
...
template<template<class> class Field, class Type>
Foam::IOFieldField<Field,Type>::IOFieldField(const IOobject& io, const FieldField<Field,Type>& f)
:
    regIOobject(io),
    ...
    field0Ptr_(NULL)
{
    ...
    if (io.readOpt() == IOobject::MUST_READ_IF_MODIFIED)
    {
        WarningIn("IOFieldField::IOFieldField(const IOobject&, const FieldField<Field,Type>&)")
            << "IOFieldField " << name()
            << " constructed with IOobject::MUST_READ_IF_MODIFIED"
            " but IOFieldField does not support automatic rereading."
            << endl;
    ...
    field0Ptr_(NULL)
{
    ...
    if (io.readOpt() == IOobject::MUST_READ_IF_MODIFIED)
    {
        WarningIn
        (
            "IOFieldField::IOFieldField(const IOobject&, const Xfer<FieldField<Field,Type> >&)"
        )   << "IOFieldField " << name()
            << " constructed with IOobject::MUST_READ_IF_MODIFIED"
            " but IOFieldField does not support automatic rereading."
            << endl;
...
template<template<class> class Field, class Type>
void Foam::IOFieldField<Field,Type>::operator=(const IOFieldField<Field,Type>& rhs)
{
...
template<template<class> class Field, class Type>
void Foam::IOFieldField<Field,Type>::operator=(const FieldField<Field,Type>& rhs)
{
    FieldField<Field,Type>::operator=(rhs);
}
