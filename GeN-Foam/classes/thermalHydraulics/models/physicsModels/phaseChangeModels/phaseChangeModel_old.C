...
Foam::scalar Foam::phaseChangeModel::relaxationFactor() const
{
    ...
    if (mesh_.relaxField("massTransfer"))
    {
        ...
        if 
        (
            !stabilizedMassTransfer and pimple_.finalIter()
        )   f = 1.0;
    }
...
void Foam::phaseChangeModel::limitMassTransfer()
{
    ...
    */
    if (pimple_.dict().found("limitMassTransferIfContErr>"))
    {
        scalar maxAbsContErr
        (
            pimple_.dict().get<scalar>
            (
                "limitMassTransferIfContErr>"
            )
        );
        ...
        (
            ...
            (
                "limitMassTransferSafetyFactor", 1.0
            )
        );

        scalar dt(mesh_.time().deltaT().value()/safetyFactor);
        ...
        forAll(dmdt_, i)
        {
            if (contErrNorm1[i] > maxAbsContErr) 
            {
                scalar& dmdt(dmdt_[i]);
                if (dmdt > 0)
                {
                    scalar maxDmdt
                    (
                        max(fluid1_[i]*fluid1_.rho()[i]/dt-div1[i], 0.0)
                    );
                    if (dmdt > maxDmdt)
                    {
                        if (pimple_.finalIter())
                            dmdtLostToAdjustments_ += (dmdt-maxDmdt)*V[i]*dt;
                        dmdt = maxDmdt;
                    }
                }
            }
            if (contErrNorm2[i] > maxAbsContErr)
            {
                scalar& dmdt(dmdt_[i]);
                if (dmdt < 0)
                {
                    scalar minDmdt
                    (
                        min(-fluid2_[i]*fluid2_.rho()[i]/dt+div2[i], 0.0)
                    );
                    if (dmdt < minDmdt)
                    {
                        if (pimple_.finalIter())
                            dmdtLostToAdjustments_ += (dmdt-minDmdt)*V[i]*dt;
                        dmdt = minDmdt;
                    }
                }
            }
        ...
        if (pimple_.finalIter())
        {
            Info<< "Cumulative dmdt lost to limiter = " 
                << (dmdtLostToAdjustments_/totV) 
                << " kg/m3" << endl;
        }
    }
    else if 
    ...
    )
    {
        ...
        scalar posDmdtIntegraldV(0.0);
        scalar posLimitedDmdtIntegraldV(0.0);
        scalar negDmdtIntegraldV(0.0);
        scalar negLimitedDmdtIntegraldV(0.0);
        scalar dmdtAvgBeforeAdj(0.0);
        if (pimple_.finalIter())
        {
            dmdtAvgBeforeAdj = dmdt_.weightedAverage(mesh_.V()).value();
        }
        ...
        forAll(dmdt_, i)
        {
            ...
            if (dmdt > 0.0)
            {
                ...
                posDmdtIntegraldV += dmdt*V[i];
                dmdt = min(dmdt, maxDmdt);
                posLimitedDmdtIntegraldV += dmdt*V[i];
            }
            else if (dmdt < 0.0)
            {
                ...
                negDmdtIntegraldV += dmdt*V[i];
                dmdt = max(dmdt, minDmdt);
                negLimitedDmdtIntegraldV += dmdt*V[i];
            }
        }

        //- Re-absorb lost dmdt in total dmdt integral to allow to spatially
        //  re-distribute the losses
        scalar adj(dmdtLostToAdjustments_*totV/dt);
        if (adj > 0.0)
        {
            negDmdtIntegraldV -= adj;
        }
        else
        {
            posDmdtIntegraldV -= adj;
        }
        ...
        (
            ...
            (
                ...
                min
                (
                    posDmdtIntegraldV/max(posLimitedDmdtIntegraldV, 1e-69), 
                    maxFrac
                )
            )
        );
        ...
        (
            ...
            (
                ...
                min
                (
                    negDmdtIntegraldV/min(negLimitedDmdtIntegraldV, -1e-69), 
                    maxFrac
                )
            )
        );  
        ...
        if (pimple_.finalIter())
        {
            scalar dmdtAvgAfterAdj(dmdt_.weightedAverage(mesh_.V()).value());
            dmdtLostToAdjustments_ += (dmdtAvgAfterAdj - dmdtAvgBeforeAdj)*dt;
            Info<< "Cumulative dmdt lost to limiter = " 
                << (dmdtLostToAdjustments_) 
                << " kg/m3" << endl;
        }
    }
...
{
    //- Limit sharp changes in interfacial temperature
    if (mesh_.relaxField("interfacialTemperature"))
    {
        bool stabilizedInterfacialTemperature
        (
            pimple_.dict().lookupOrDefault<bool>
            (
                "stabilizedInterfacialTemperature", false
            )
        );
        scalar f(1.0);
        if (!pimple_.finalIter() and !stabilizedInterfacialTemperature)
            f = mesh_.fieldRelaxationFactor("interfacialTemperature");
        iT_ = f*saturation_->Tsat(p_)()+(1-f)*iT_;
    }
    if (pimple_.dict().found("maxInterfacialTempRelChangePerIter"))
    {
        scalar maxRelChange
        (
            pimple_.dict().get<scalar>
            (
                "maxInterfacialTempRelChangePerIter"
            )
        );
