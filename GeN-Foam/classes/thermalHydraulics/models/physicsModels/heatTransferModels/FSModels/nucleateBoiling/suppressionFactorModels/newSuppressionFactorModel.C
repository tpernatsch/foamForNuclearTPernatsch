...
Foam::autoPtr
<
    Foam::heatTransferModels::nucleateBoilingSubModels::suppressionFactorModel
> 
Foam::heatTransferModels::nucleateBoilingSubModels::suppressionFactorModel::
New
(
...
{
    word type(htm.subDict("suppressionFactorModel").get<word>("type"));
    Info<< "Constructing suppressionFactorModel of type " << type 
        << " in region(s): " << FSPair.structure().regions() << endl;

    suppressionFactorModelsConstructorTable::iterator cstrIter =
        suppressionFactorModelsConstructorTablePtr_->find
        (
            type
        );
    ...
    if (cstrIter == suppressionFactorModelsConstructorTablePtr_->end())
    {
        FatalErrorInFunction
            << "Unknown type type "
            << type << endl << endl
            << "Valid suppressionFactorModel types are : " << endl
