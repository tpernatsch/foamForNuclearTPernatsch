    ...
    firstTimeStepAndIter_(true)
{
    ...
    //- Read physicsModels in phaseProperties dictionary
...
)
{
    ...
    if (correctEnergy)
    {
        ...
        {
            //- Limits interfacial area then updates interfacial temperature, 
            //  latent heat and mass transfer. Also, sets the interfacial heat
            //  transfer coefficients to 0 in boiling regions (i.e. mag(dmdt) 
            //  > 1e-3).
            phaseChangePtr_->correct();
        }
        else
        {
            //- Update interfacial temperature so to ensure energy conservation
