...
:
    ...
    (
        ...
        mesh,
        dimensionedScalar("0", dimVol/dimTime, 0)
    ),
    alphaRhoPhi_
    (
        IOobject
    ...
    )
{
    ...
    if (dict_.found("initialAlphas"))
    {
        ...
        )
        {
            ...
            scalar alpha0(alpha0s.lookupType<scalar>(zoneName));
            forAllConstIter
            (
                labelList,
    ...
    //  boundary conditions of phi (and initializing the turbulence model)
...
void Foam::fluid::correctInflowOutflow(surfaceScalarField& alphaPhi) const
{
