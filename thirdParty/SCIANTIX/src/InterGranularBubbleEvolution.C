...
void InterGranularBubbleEvolution( )
{
  ...
  Intergranular_bubble_concentration[1] = (std::max(Intergranular_bubble_concentration[0], 1e-150));

  // Gas is distributed among bubbles
  const double grain_surface_to_volume_ratio = 3.0 / Grain_radius[1];
  Intergranular_atoms_per_bubble[1] = std::max(Gas_boundary[1], 0.0) / (Intergranular_bubble_concentration[1] * grain_surface_to_volume_ratio);
  ...
  {
    //TO DO: here should be Temperature[1]
    const double vacancy_diffusion_coefficient = GrainBoundaryVacancyDiffusionCoefficient(Temperature[0]);
    Intergranular_fractional_coverage[0] = (std::max(Intergranular_bubble_concentration[0], 1e-150)) * Intergranular_bubble_area[0];
    const double sink_strength = -0.25*((3.0-Intergranular_fractional_coverage[0])*(1.0-Intergranular_fractional_coverage[0])+2.0*log(std::max(Intergranular_fractional_coverage[0], 1e-150))); // (/)
    ...
    //TO DO: here should be Hydrostaticstress[1]
  ...
  // Deduce minimum rateDotTimeStep allowed (i.e. the one causing a concentration of 4e13)
  double dAreaMin = (Intergranular_bubble_concentration[0] - 4.0e+13)/4.0e+13/geometric_coefficient/Intergranular_bubble_concentration[0];

  Intergranular_bubble_concentration[1] = Solver::BinaryInteraction(Intergranular_bubble_concentration[0], geometric_coefficient, std::max(dbubble_area, dAreaMin));


  Intergranular_atoms_per_bubble[1] *= (std::max(Intergranular_bubble_concentration[0], 1e-150)) / Intergranular_bubble_concentration[1];
  Intergranular_vacancies_per_bubble[1] *= (std::max(Intergranular_bubble_concentration[0], 1e-150)) / Intergranular_bubble_concentration[1];
  Intergranular_bubble_volume[1] = Intergranular_atoms_per_bubble[1] * Xenon_covolume + Intergranular_vacancies_per_bubble[1] * Vacancy_volume;
  Intergranular_bubble_radius[1] = pow(3.0*Intergranular_bubble_volume[1]/(4.0*Pi*lenticular_shape_factor),1.0/3.0);
  Intergranular_bubble_area[1] = Pi*pow(Intergranular_bubble_radius[1]*sin(semidihedral_angle), 2);
