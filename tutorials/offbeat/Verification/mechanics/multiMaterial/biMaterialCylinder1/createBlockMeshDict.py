...
radii = np.linspace(1, 2, 3)

va = [NewVertex(r, 0 ,0).rotZ(-45) for r in radii]
vb = [v.rotZ(90) for v in va]
vc = [v + (0,0,0.1) for v in va]
vd = [v + (0,0,0.1) for v in vb]

arcs = [NewArcEdge(a, b, a.rotZ(45)) for a, b in zip(va, vb)]
arcs += [NewArcEdge(a, b, a.rotZ(45)) for a, b in zip(vc, vd)]
...
blockNames = ['inner', 'outer']
blocks = []
for name, vertices in zip(blockNames, zip(va[:-1], va[1:], vb[1:], vb[:-1], 
                                          vc[:-1], vc[1:], vd[1:], vd[:-1])):
    blocks.append(bmd.addHexBlock(vertices, (25, 59, 1), name=name))

frontAndBackFaces = [face for b in blocks for face in [b.face('b'), b.face('t')]]
symm1Faces = [b.face('n') for b in blocks]
symm2Faces = [b.face('s') for b in blocks]
bmd.add_boundary('empty', 'frontAndBack', frontAndBackFaces)
bmd.add_boundary('patch', 'inner', [blocks[0].face('w')])
bmd.add_boundary('patch', 'outer', [blocks[-1].face('e')])
bmd.add_boundary('patch', 'symmetry1', symm1Faces)
bmd.add_boundary('patch', 'symmetry2', symm2Faces)
