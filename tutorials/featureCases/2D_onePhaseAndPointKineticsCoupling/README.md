...
-   like the transient above, but including the feedback from an assumed
...
In both transients, the time steps are adjusted so to keep a power change of
~ 1% per time step.
...
after, the time scales are compatible with fuel and driveline heat-up scales,
...
-   constant/neutroRegion/reactorState. This file is only used to specify the
    ...
    The initial precursor concentrations can be specified via the
