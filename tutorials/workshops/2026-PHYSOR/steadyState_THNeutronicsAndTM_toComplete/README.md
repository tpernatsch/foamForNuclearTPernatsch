...
1. **Complete the regionsDict dictionary.** In regionDict, change the regionSolvers and mappings entries to correctly set-up the physics of the problem. In order to select the correct coupling fields, please refer to the field names table in https://foamfornuclear.gitlab.io/foamForNuclear/usersguide/GeN-Foam/coupling.html
...
3. **Plot Results** Run the "plotResults.py" file to study the convergence of k-eff and fuel and coolant temperatures. Compare the results to those in which core expansions were not included with "compareKeff.py".
...
```
This will tightly coupled the physics together, performing Picard iterations until the convergence criterion is met. 