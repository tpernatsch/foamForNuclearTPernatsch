...
# ---------------------------------------------------------------------------
# Power density, buoyancy, and turbulence fields (provided — no task here)
# ---------------------------------------------------------------------------
powerDensityStructure = ffn.Field("powerDensityStructure", region=thMesh.region)
powerDensityStructure.dimensions   = ffn.Dimension(1, -1, -3, 0, 0, 0, 0)
powerDensityStructure.internalField = 0
for patch in ['wall', 'baffle0', 'baffle1', 'top', 'bottom']:
    powerDensityStructure.set_boundary_condition(patch, bc.ZeroGradient())

powerDensityLiquid = ffn.Field("powerDensityLiquid", region=thMesh.region)
powerDensityLiquid.dimensions   = ffn.Dimension(1, -1, -3, 0, 0, 0, 0)
powerDensityLiquid.internalField = 0
for patch in ['wall', 'baffle0', 'baffle1', 'top', 'bottom']:
    powerDensityLiquid.set_boundary_condition(patch, bc.ZeroGradient())
...
timeFolder0.append(powerDensityStructure)
timeFolder0.append(powerDensityLiquid)
