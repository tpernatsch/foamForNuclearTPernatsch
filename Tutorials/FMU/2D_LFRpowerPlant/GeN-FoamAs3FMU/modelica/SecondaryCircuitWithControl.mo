model SecondaryCircuitWithControl
  ...
  Modelica.Blocks.Sources.Constant frequencyCmd(k = system.fnom) annotation(
  ...
  // 0.125*Tu; //
  Modelica.Blocks.Continuous.LimPID pidFrequency(Td = Td, Ti = Ti, controllerType = Modelica.Blocks.Types.SimpleController.PID, initType = Modelica.Blocks.Types.InitPID.InitialOutput, k = Kp, limitsAtInit = true, yMax = 1, yMin = 0, y_start = 0.285) annotation(
...
equation
  ...
  connect(frequencySensor.port, generator.port) annotation(
    Line(points = {{160, 100}, {174, 100}, {174, 60}, {168, 60}}, color = {0, 0, 255}));
  ...
  connect(pidFrequency.u_s, frequencyCmd.y) annotation(
    Line(points = {{112, 140}, {139, 140}}, color = {0, 0, 127}));
  ...
  connect(pidFrequency.y, valveVapAdmissionTurb.theta) annotation(
    Line(points = {{90, 140}, {-90, 140}, {-90, 84}}, color = {0, 0, 127}));
  connect(pidFrequency.u_m, frequencySensor.f) annotation(
    Line(points = {{100, 128}, {100, 100}, {140, 100}}, color = {0, 0, 127}));
...
Circuit")}),
    version = "");
end SecondaryCircuitWithControl;