...
with open("log.GeN-Foam", "r") as log:
    for line in log:
        ...
        if time_match:
            if current_time is not None:
                if last_keff is not None:
                    ...
                    keff_vals.append(last_keff)  # <<< ADDED
        ...
        # k-eff match
        keff_match = re.match(r"keff_\s*=\s*([0-9Ee\.\+-]+)", line)  # <<< ADDED
        if keff_match:
            last_keff = float(keff_match.group(1))  # <<< ADDED

# Append last time step
if current_time is not None and last_keff is not None:
    times.append(current_time)
    fuel_Tmax.append(last_fuel_Tmax)
    clad_Tmax.append(last_clad_Tmax)
    fluid_Tmax.append(last_fluid_Tmax)
...
save_plot(times, keff_vals, 'k-eff', 'Effective Multiplication Factor vs Time', 'keff_vs_time.png', 'purple')  # <<< ADDED
