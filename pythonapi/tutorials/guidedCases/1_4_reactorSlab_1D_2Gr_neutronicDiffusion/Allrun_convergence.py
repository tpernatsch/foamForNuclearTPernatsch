...
k_rel_errors = [abs(k_sim - k_analytical) / k_analytical * 100 for k_sim in results['keff']]
...
fig1, (ax_L2_fast, ax_L2_thermal) = plt.subplots(nrows=2, figsize=(5, 6), dpi=200)
...
ax_L2_fast.legend()
ax_L2_fast.grid(True, alpha=0.3)
...
ax_L2_thermal.grid(True, alpha=0.3)

fig1.tight_layout()
fig1.savefig("fig_results_L2_convergence.png")
plt.close()
...
fig2, (ax_keff, ax_kerr) = plt.subplots(nrows=2, figsize=(5, 6), dpi=200)

# k_eff convergence
ax_keff.plot(results['nz_values'], results['keff'], 'o-', linewidth=2, markersize=8, label='GeN-Foam')
ax_keff.axhline(y=k_analytical, color='tab:red', linestyle='--', linewidth=2, label=f'Analytical ({k_analytical:.6f})')
...
ax_kerr.axhline(y=0, color='black', linestyle='--', linewidth=0.8)
ax_kerr.set_xlabel("Number of axial cells (nz) [-]")
ax_kerr.set_ylabel("Relative error [%]")
ax_kerr.set_xscale('log')
ax_kerr.set_yscale("log")
ax_kerr.grid(True, alpha=0.3)

fig2.tight_layout()
fig2.savefig("fig_results_keff_convergence.png")
plt.close()
...
fig3, (ax_rel_err_fast, ax_rel_err_thermal) = plt.subplots(nrows=2, figsize=(5, 6), dpi=200)
...
for i, nz in enumerate(nz_values):
    ...
    ax_rel_err_fast.plot(z, rel_error_0, label=f'nz={nz}', alpha=0.7)

ax_rel_err_fast.set_xlabel("Axial position [m]")
ax_rel_err_fast.set_ylabel("Relative error [%]")
ax_rel_err_fast.set_title("Relative Error vs Position (Group 0 - Fast)")
ax_rel_err_fast.legend()
ax_rel_err_fast.grid(True, alpha=0.3)
ax_rel_err_fast.set_xlim(0, fuelLength * 1.05)
...
for i, nz in enumerate(nz_values):
    ...
    ax_rel_err_thermal.plot(z, rel_error_1, label=f'nz={nz}', alpha=0.7)

ax_rel_err_thermal.set_xlabel("Axial position [m]")
ax_rel_err_thermal.set_ylabel("Relative error [%]")
ax_rel_err_thermal.set_title("Relative Error vs Position (Group 1 - Thermal)")
ax_rel_err_thermal.legend()
ax_rel_err_thermal.grid(True, alpha=0.3)
ax_rel_err_thermal.set_xlim(0, fuelLength * 1.05)

fig3.tight_layout()
fig3.savefig("fig_results_relative_errors.png")
plt.close()


# Plot 4: Flux distribution comparison for both groups
#-----------------------------------------------------

fig4, ax_flux = plt.subplots(figsize=(5, 4), dpi=200)

for i, nz in enumerate(nz_values):
    z = results['z_mesh'][i]
    flux0 = results['flux0'][i]
    flux1 = results['flux1'][i]
    ax_flux.plot(z, flux0, 'o-', label=f'Fast nz={nz}', alpha=0.7, markersize=3)
    ax_flux.plot(z, flux1, 's-', label=f'Thermal nz={nz}', alpha=0.7, markersize=3)
...
ax_flux.set_xlim(0, fuelLength * 1.05)

fig4.tight_layout()
fig4.savefig("fig_results_flux_comparison.png")
plt.close()
...
print(tabulate(
    table_data,
    headers=['nz', 'k_eff', 'k_err [%]', 'L2 err g=0', 'L2 err g=1'],
    tablefmt='pipe'
))
