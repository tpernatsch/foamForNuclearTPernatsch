"""
Mesh refinement study for 1D slab reactor with external source.
...
for nz in nz_values:
    ...
    # Extract flux data
    time_extract = source_table.table[-2][0]
...
flux_analytical = get_analytical_solution(k_inf, L_squared, H, S0, D)

# Calculate errors for each mesh (using normalized flux)
L2_errors = []
Linf_errors = []
...
for i, nz in enumerate(nz_values):
    ...
    # Transform coordinates for analytical solution
    z_centered = z - H/2
    flux_ana = flux_analytical(z_centered)
...
z_analytical_centered = z_analytical - H/2
flux_analytical_refined = flux_analytical(z_analytical_centered)
...
ax_L2.set_title("L2 Error Convergence - Normalized Flux (k$_{inf}$ = " + f"{k_inf:.4f})")
ax_L2.legend()
...
ax_Linf.set_title("L∞ Error Convergence - Normalized Flux (k$_{inf}$ = " + f"{k_inf:.4f})")
ax_Linf.legend()
...
ax_rel_err.set_title("Rel. Error vs Position - Normalized Flux (k$_{inf}$ = " + f"{k_inf:.4f})")
ax_rel_err.legend()
ax_rel_err.set_xlim(0, H)
...
for i, nz in enumerate(nz_values):
    ...
    ax_flux.plot(z, flux0_norm, '-', label=f'nz={nz}', alpha=0.7, markersize=3)

ax_flux.plot(z_analytical, flux_analytical_refined_norm, 'k--', linewidth=2, label=f'Analytical (k_inf={k_inf:.4f})')
ax_flux.set_xlabel("Axial position [m]")
ax_flux.set_ylabel("Normalized neutron flux [-]")
ax_flux.set_title("Normalized Flux Dist. Comparison (k$_{inf}$ = " + f"{k_inf:.4f})")
ax_flux.legend()
