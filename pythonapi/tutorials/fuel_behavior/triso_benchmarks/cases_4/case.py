...
def _build_pyc_material(cfg):
    m    = _MAT["IPyC"]
    Tref = cfg["pyc_Tref"]

    # Swelling
    sw_type = cfg["pyc_swelling"]
    if sw_type == "constant":
        swelling = behaviour.swelling.ConstantRate(swellingRate=cfg["pyc_swelling_rate"])
    elif sw_type == "correlation":
        sw = PYC_SWELLING_COEFFICIENTS[cfg["pyc_swelling_set"]]
    ...
    # Constitutive law
    creep_type = cfg["pyc_creep"]
    if creep_type == "constant":
        constitutiveLaw = laws.MisesPlasticCreep(
            yieldStress=laws.yield_stress.Constant(sigmaY=1e60),
            creep=laws.creep.ConstantPrincipalStress(
                creepCoefficient=cfg["pyc_creep_coefficient"]
            ),
        )
    else:
        constitutiveLaw = laws.Elasticity()
...
def build_case(case_id: str = "case_4a"):
    ...
    case.materials = [
        _build_pyc_material(cfg) if name == "IPyC"
