...
from foamForNuclear.common import Vector, Table
...
def _build_pyc_material(name, Tref, swelling_set, creep_coefficients):
    m  = _MAT[name]
    ...
    return materials.PyC(
        ...
        constitutiveLaw=laws.MisesPlasticCreep(
            ...
            creep=laws.creep.CorrelationPrincipalStress(
                coefficientList=creep_coefficients, fluxConversionFactor=1.0,
            ),
        ),
    )


def build_case(case_id: str = "case_8"):
    ...
    T_table = Table(cfg["T_list"])

    T = fields.Temperature(
        internalField=873.0,
        boundaryField=dict(
            inner=bc.UniformFixedValue(uniformValue=T_table),
            outer=bc.UniformFixedValue(uniformValue=T_table),
        )
    )
    ...
    Tref   = cfg["pyc_Tref"]
    creep  = cfg["pyc_creep_coefficients"]
    sw     = cfg["pyc_swelling_set"]

    case.materials = [
        _build_pyc_material(name, Tref, sw, creep) if name in ("IPyC", "OPyC")
    ...
    case.settings.writeInterval = 8.64e6
