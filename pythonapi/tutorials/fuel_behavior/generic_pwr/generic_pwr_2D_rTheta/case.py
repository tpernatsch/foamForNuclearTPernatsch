...
def build_case():
    # %% 1) Geometry and mesh
    rod_mesh = mesh.UnvMesh(
        srcpath="PelletDiscCoarser.unv",
        special_patches={
            "top": "empty",
            "bottom": "empty",
            "front": "symmetry",
    ...
    rod_mesh.transforms.append(("scale", (0.001, 0.001, 0.001)))
    rod_mesh.transforms.append(("translate", (0.0, 0.0, 1.5)))
    ...
    case.globalOptions.angularFraction = 0.25
    ...
    # NOTE: One can add a small gap volume offset to represent an approximate plenum 
    # (proportional to the disc height)
    ...
    fuelOuter = functions.PatchProbes(
        ...
        probeLocations=[[fuel_ro, 0.0, 1.5005]],
        patches=["fuelOuter",],
        enabled=True
    )
    ...
    radialProfile = functions.Graph(
        name="radialProfile",
        start=[0.0, 0.0, 1.5005 ],
        end=[fuel_ro, fuel_ro,  1.5005],
        fields=['T', 'sigma'],
        nPoints=100,
