...
if __name__ == "__main__":
    ...
    ip, points = fuel_outer.read_from_case(
        startTime=0,
        fieldName="interfaceP"
    )
    
    hGap, points = fuel_outer.read_from_case(
        startTime=0,
        fieldName="hGap"
    )
    ...
    ip_list = [val[0]/1000000 for val in ip.values()]
    hGap_list = [val[0] for val in hGap.values()]
    ...
    # ---- Interface pressure (right axis #3, offset) ----
    ax_ip = ax_temp.twinx()
    ax_ip.spines['right'].set_position(('axes', 1.4))  # shift to avoid overlap
    ax_ip.spines['right'].set_visible(True)             # make sure it draws
    l4, = ax_ip.plot(time, ip_list, label="Contact pressure", color='tab:olive')
    ax_ip.tick_params(axis='y', labelcolor='tab:olive')
    ax_ip.set_ylabel('Contact pressure [MPa]', color='tab:olive')

    # ---- Gap conductance (right axis #4, offset) ----
    ax_hGap = ax_temp.twinx()
    ax_hGap.spines['right'].set_position(('axes', 1.6))  # shift to avoid overlap
    ax_hGap.spines['right'].set_visible(True)             # make sure it draws
    l5, = ax_hGap.plot(time, hGap_list, label="Gap conductance", color='tab:purple')
    ax_hGap.tick_params(axis='y', labelcolor='tab:purple')
    ax_hGap.set_ylabel('Gap conductance [W/m$^2$K]', color='tab:purple')
