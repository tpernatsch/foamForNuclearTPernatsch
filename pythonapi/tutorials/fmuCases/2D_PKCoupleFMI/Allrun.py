...
thSolver.fluid_structure.heatTransferModels.append(
    ffn.porous_medium.heat_transfer.NusseltReynoldsPrandtlPower(
        const=4.82, coeff=0.0185, expRe=0.827, expPr=0.827, zones=['zone0'])
)
...
fieldIntegralToFMU = ffn.preprocessing.FieldIntegralToFMU(
    ...
    region="neutroRegion",
    cellZone="zone0",
    initValue=neutronicsSolver.power
)
...
ffn.copyFolder('ExternalReactivityController.py', model.caseFolder)
ffn.copyFolder('ExternalReactivityController.fmu', model.caseFolder)
...
fig, ax = plt.subplots()
ax.plot(res['time'], [e * 1e-6 for e in res['totalPower']])
