...
The ``sliceMapper`` model defines axial slices that allow 1-D slice-based
operations to be performed on a 3-D mesh in the thermo-mechanics module
...
In these situations, slice-averaged quantities must be computed in a way
that remains compatible with traditional fuel performance approaches.

However, OpenFOAM does not natively provide a concept of axial slices,
and such a concept cannot be directly generalized to arbitrary
3-D domains. The ``sliceMapper`` class addresses this by defining a
mapping between the 3-D mesh cells and a set of virtual 1-D axial slices.
...
If the ``sliceMapper`` dictionary is not present in ``solverDict``,
no slice mapping is created. Some models that rely on slice-averaged
