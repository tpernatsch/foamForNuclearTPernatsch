...
simple block-structured meshes, and OpenFOAM’s built-in ``blockMesh`` utility is often
sufficient.

``blockMesh`` builds the computational mesh based on the ``blockMeshDict`` dictionary located in the
``system/`` directory. Executing this command creates the ``constant/polyMesh/`` folder,
which stores all mesh information (points, faces, cells, etc.) in a set of dictionaries.
...
and files in the casefolder. By default, it reads instructions from ``system/changeDictionaryDict``. 
...
presence of the ``end`` string at the end of the output, which should always be checked.
