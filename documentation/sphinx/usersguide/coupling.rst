...
GeN-Foam has been developed for the steady-state and transient analysis of
reactors featuring pin-type, plate-type, or liquid fuel (viz., Molten Salt
Reactors). It includes sub-solvers for neutronics, single- and two-phase
thermal-hydraulics, thermal-mechanics and fuel-behaviour, via the newly imported
libraries of the fuel behaviour code
`OFFBEAT <https://gitlab.com/foam-for-nuclear/offbeat>`_. An arbitrary number of
*regions* can be created, each associated to a different mesh and physics. The
selection of the physics to solve is done in *system/controlDict*, in the
*regionSolvers* entry.


.. _userguide_coupling_the-controlDict-dictionary:

The *controlDict* dictionary
----------------------------

The *controlDict* is an extended version of the one that is normally used in
