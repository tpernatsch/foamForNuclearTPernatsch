...
.. warning::
   ...
   **must** be defined for ``DD`` (not for ``D``). In such cases, ``D`` does not need to
   be present in the initial time folder, because it is reconstructed during the simulation.
   The linear solver settings (and residual controls) must also be provided for
