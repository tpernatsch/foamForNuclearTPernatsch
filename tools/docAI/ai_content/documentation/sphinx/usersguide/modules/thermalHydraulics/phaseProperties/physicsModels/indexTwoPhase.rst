...
The *physicsModels* sub-dictionary is used in two-phase simulations to define the models that describe the interaction between fluid1 and structure, fluid2 and structure, and between fluid1 and fluid2. Several types of models can be defined:
...
Each of these sub-dictionaries consists of a series of other sub-dictionaries. Their properties are applied to the cell zones whose names are given in the sub-dictionary keys. Three options exist:

- Use ``constant`` as the ``type`` and provide a constant value using the keyword ``value``.
- Use a single model as the ``type``, chosen among those available.
- Use ``byRegime`` as the ``type``, and then choose one of the available models for each regime. This requires setting a :ref:`regime map <modules_thermalHydraulics_porousMedium_regimeMapModels>`.

An example of a dictionary is reported below.
...
.. note::
    ...
    Anisotropic pressure drops can be set using three different correlations for the three different local axes (see :ref:`the physicsModels sub-dictionary <modules_thermalHydraulics_porousMedium_physicsModels>`). In addition, it is possible to use an anisotropic hydraulic diameter (see :ref:`the structureProperties sub-dictionary <modules_thermalHydraulics_porousMedium_structureProperties>`). The anisotropy of the hydraulic diameter can be set using the keyword *localDhAnisotropy* and assigned a vector of three scaling factors, one for each local direction.