...
The steady state corresponds to the KIWI-B-4E configuration at time 20800 s, a few seconds before the shutdown transient [3]. Then follows the shutdown transient, which relies on the point-kinetics neutronics sub-solver.
...
There are currently 10 fuel element types placed with the same fuel loading pattern as in OpenMC. This model also includes unloaded central elements (with and without Tantalum), as well as upper and lower support plates.
...
The fuel elements are merged into a single cellZone, as are the unloaded central elements. The mesh also includes clear fluid plena at the inlet and outlet. This model uses the specially developed hydrogen thermophysical properties model, including hydrogen dissociation at high temperature.
...
The script to generate the mesh for the full-core model is in the [coreMeshGenerator.py](mesh/coreMeshGenerator.py) file. This Python script is based on the Salome Python API. It reads a `.txt` file containing the lattice map of the core. In this case, it is [coreMap.txt](mesh/coreMap.txt).
...
Load the following script to generate the mesh and export `neutroMesh` and `fluidMesh` in **UNV** format into the `mesh` folder.
...
*Fig 4. Side-by-side comparison of the KIWI-B-4E neutronics and thermal-hydraulics meshes, visualized in ParaView.*


## Boundary conditions

In the fluid region, the boundary conditions are fixed values of `U` and `p` at the inlet and outlet, respectively. Fixed values for `k` and `epsilon` at the inlet have been computed using the [`AllcomputeInitialBCValues.py`](./AllcomputeInitialBCValues.py) script:
...
It is recommended to run the case in parallel for faster simulation. The current parallelization setup uses 8 physical cores, but it can be increased in [`Allrun_parallel`](./Allrun_parallel).
...
*Fig 7. Fuel temperature field in the RZ projection, with a comparison to experimental data from Ref. [3].*


<img src="./images/3D_KIWI-B-4E_fig_results_transientSummary.png" width="350px">

*Fig 8. Evolution of power, reactivity, temperature, and mass flow rate during the shutdown of the KIWI-B-4E GeN-Foam core model.*


![](./images/3D_KIWI-B-4E_fig_results_transientShutdown.gif)

*Fig 9. Evolution of hydrogen temperature and power density during the shutdown transient. The slices show fast and thermal flux distributions.*


## Publication

This tutorial and the presented results are the subject of the following journal paper, which is recommended for citation:
