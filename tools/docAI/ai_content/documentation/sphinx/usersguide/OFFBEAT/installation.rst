...
distributions. It should compile and run without problems on any Linux system
...
The OFFBEAT compilation script automatically identifies which OpenFOAM
environment version is currently sourced.
...
Use the ``git checkout`` command to change branches. For example, to switch
...
directory ``$FOAM_USER_APPBIN``. Once compilation is complete, OFFBEAT should
