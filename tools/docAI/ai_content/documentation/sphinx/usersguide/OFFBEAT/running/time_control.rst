...
When the switch is disabled, all time-step criteria described below are ignored. The
simulation then advances with the fixed ``deltaT`` specified in ``controlDict``.
...
* ``maxDeltaMatTol``  
  Maximum tolerated relative variation of material properties (e.g. :math:`\rho`,
  :math:`C_p`, :math:`k`, emissivity, :math:`E`, :math:`\nu`, :math:`\alpha`). Default:
  ``GREAT``.
