# Bi-material cylinder 2

Two concentric cylinders made of materials with different mechanical properties are fixed at the inner surface and subjected to a torque of 1 Nm. The case layout is shown below.

<div style="text-align: center;"><figure>
<img src="layout.svg" alt="run test to generate figure" width="100%">
<figcaption>Bi-material block case layout</figcaption>
</figure></div>

For this case, the shear stress \( \sigma_{r\theta} \) is constant across the interface, while the other stress components are zero.

<div style="text-align: center;"><figure>
<img src="sigma.png" alt="run test to generate figure" width="75%">
<figcaption>Stress distribution in the x-direction</figcaption>
</figure></div>

Conversely, the shear strain \( \epsilon_{r\theta} \) is discontinuous across the interface.

<div style="text-align: center;"><figure>
<img src="epsilon.png" alt="run test to generate figure" width="75%">
<figcaption>Strain distribution in the x-direction</figcaption>
</figure></div>