...
In GeN-Foam, the number of selected regions, each with its own mesh and solver,
is completely arbitrary and specified by the user in the *system/regionsDict*
...
selected regions are achieved independently, without exchanging information
across them. The following sections detail methods for coupling physics.
...
Volumetric coupling is best suited for geometrically overlapping domains. It is
achieved by volumetrically mapping fields from one mesh to another, using
...
*regionsDict/mappings/fluidRegion/neutroRegion*. Within this *subDict*, the
name of the field on the source mesh is specified in the *sourceFields* entry
(e.g., ``powerDensity`` in the neutroRegion). The name of the field on the
target mesh is provided in the *targetFields* entry (e.g.,
...
has been created to allow users to experiment with volumetric couplings and
understand their logic.

.. note ::
    GeN-Foam can be used to map *any* scalar or vectorial field to *any* scalar
    or vectorial field on a different mesh. This enables the simulation of
    arbitrarily coupled multi-physics problems that leverage the currently
    existing libraries.

.. note ::
    There is no need for all domains to be the same. GeN-Foam will project
    fields in a "clever" way whenever there is no overlap between two domains.
...
formulation. This result is stored in the field named ``disp``. In order to compute the
neutronics deformation, the radial component of ``disp`` is combined with an axial
...
map the coolant phase from TH to neutronics) are single-phase. Therefore, simply
mapping ``alpha`` (TH) to ``alpha`` (neutronics) will work. However, in the case of a
...
OpenFOAM functions, one can create a new field and map it if needed. An
example of such an approach is shown below.
...
located in the *0* directory. These specifications can involve simple mappings
or more advanced coupling strategies, such as conjugate heat transfer, where
...
Many multi-physics simulations require large flexibility in the
...
simulations, where physics are tightly coupled within a scale and loosely coupled
across scales. To create this additional flexibility, dedicated "loops" are
created.

For example, the ``picardLoop`` consists of a list of tightly coupled physics.
During run-time, when the physics are corrected, if they are part of a
``picardLoop``, the physics will be corrected iteratively until a user-input
convergence criterion is met. At each iteration of the loop, volumetric and
boundary field exchanges are performed to update the coupling variables between
the involved regions.

In addition to the standard ``picardLoop``, other loop types exist that are
designed for specific coupling strategies. They may involve additional field
manipulations or residual evaluations tailored to the physics being coupled.
*loops* are also defined in *system/regionsDict* in the ``regionSolvers``
sub-dictionary. Each entry of the subDict corresponds to a different ``loop``.
It is characterized by a list of subSolvers (i.e., the solvers that are tightly
coupled), a minimum residual to reach before the iterations are interrupted, and
a maximum number of iterations. It may also include additional keywords,
depending on the type of *loop* selected.

Note that *loops* can be nested within one another, possibly leading to a
...
Note that all the physics specified in the ``Level_0`` of the ``regionSolvers`` dict
are loosely coupled. In contrast, only those specified within a ``picardLoop`` are
tightly coupled.

The previous example corresponds to a time loop as shown below.
...
structures—fuel, cladding, control rod driveline, wrapper tubes, the diagrid,
