...
Solution control is mostly achieved using the ``controlDict``, which is an extended version of the one normally used in other OpenFOAM solvers.
...
Compared to a standard OpenFOAM ``controlDict``, it includes several specialized keywords, including:
...
An additional option for mesh manipulation called ``removeBaffles`` is available. It allows you to select the physics that require the creation of a ghost mesh without baffles. This ghost mesh enables better mesh-to-mesh projections between different physics.
